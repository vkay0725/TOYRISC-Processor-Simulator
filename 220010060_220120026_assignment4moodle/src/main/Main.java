...
public class Main {
	...
	public static void main(String[] args) {
		...
		Simulator.simulate();
		processor.printState(0, 30); // ((0, 0) refers to the range of main memory addresses we wish to print. this
										// is an empty set.
		
		Statistics.printStatistics(args[1]);

		System.out.println(
				"Hash of the Processor State = " + getHashCode(processor.getRegisterFile(), processor.getMainMemory()));
	}
	...
	static int getHashCode(RegisterFile registerState, MainMemory memoryState) {
		...
		System.out.println(registerState.getProgramCounter());

		for (int i = 0; i < 32; i++) {
			hash.add(registerState.getValue(i));
			System.out.println(registerState.getValue(i));
		}
