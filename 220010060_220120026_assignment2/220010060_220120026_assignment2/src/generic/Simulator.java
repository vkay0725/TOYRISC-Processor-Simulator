...
public class Simulator {
	...
	{
		FileOutputStream tnvk;
		try{
			//1. open the objectProgramFile in binary mode
			tnvk = new FileOutputStream(objectProgramFile);
			BufferedOutputStream mech = new BufferedOutputStream(tnvk);
			...
			for(int i=0; i<ParsedProgram.code.size(); i++){
				...
				if(opType.equals("jmp")){
					...
					if(offset>=0){
						...
						check = check.concat(join);
						check = check.concat(binRep);
					} else {
						String binRep = Integer.toBinaryString(offset);
						String subRep = binRep.substring(10, 32);
						check = check.concat(subRep);
					}
				}
				else if(opType.equals("load")||opType.equals("store")){
					check += (opTable.get(opType));
			
					if(ParsedProgram.code.get(i).sourceOperand1.operandType.toString().equals("Register")){
						check = check.concat(String.format("%5s", Integer.toBinaryString(ParsedProgram.code.get(i).sourceOperand1.value)).replace(' ', '0'));
						
					}
					if(ParsedProgram.code.get(i).destinationOperand.operandType.toString().equals("Register")){
						check = check.concat(String.format("%5s", Integer.toBinaryString(ParsedProgram.code.get(i).destinationOperand.value)).replace(' ', '0'));
					}
					if(ParsedProgram.code.get(i).sourceOperand2.operandType.toString().equals("Label")){
						...
						String labeConcat = "";
						if(labeSize != 17){
							int enumm = 17 - labeSize;
							for(int j=0; j<enumm; j++)
								labeConcat += "0";
							labeConcat = labeConcat.concat(labeVal);
						}
						check = check.concat(labeConcat);
					}
					...
					if(ParsedProgram.code.get(i).sourceOperand2.operandType.toString().equals("Register")){
						check = check.concat(String.format("%5s", Integer.toBinaryString(ParsedProgram.code.get(i).sourceOperand2.value)).replace(' ', '0'));
					}
					if(ParsedProgram.code.get(i).destinationOperand.operandType.toString().equals("Register")){
						check = check.concat(String.format("%5s", Integer.toBinaryString(ParsedProgram.code.get(i).destinationOperand.value)).replace(' ', '0'));
					}
					check = check.concat("000000000000");
				}
...
labeConcat = labeConcat.concat(labeVal);
					...
					check = check.concat("11101000000000000000000000000000");
				}
				System.out.println(check);
				int b_int = (int)Long.parseLong(check, 2);
